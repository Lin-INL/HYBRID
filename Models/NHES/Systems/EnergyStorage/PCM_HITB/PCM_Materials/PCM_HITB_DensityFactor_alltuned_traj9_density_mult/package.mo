...
package PCM_HITB_DensityFactor_alltuned_traj9_density_mult "PCM_HITB_using_sin_and_cos"
  ...
  constant Real melt_band(unit = "K") = 20;
                                        //melting band for phase change
  constant Modelica.Units.SI.Temperature T_melt = 743.15;
  ...
  constant Real b1(unit = "J/(kg.K)") = 800.0;
//  constant Real b1(unit = "J/(kg.K)") = 1500;
 ...
 // constant Real b3(unit = "J/(kg.K)") = 1588.57;
  ...
  constant Real h_melt(unit = "J/kg") = 250000;
  ...
  //constant Real fT(unit = "1/K") = 2*Modelica.Constants.pi/(T_meltplus-T_meltmin);
                                                                                  //New constant
  constant Real k_slope_mult(unit = "W/m.K.K") = -0.00268;
                                                //slope for conductivity
  constant Real k_mult_solid(unit = "W/m.K") = 56;
                                                //intercept for solid conductivity
  constant Real k_mult_liquid(unit = "W/m.K") = 55.85;
                                                //intercept for liquid conductivity
  constant Real rho_solid(unit = "kg/m.m.m") = 2600;
                                                //solid density
  constant Real rho_liquid(unit = "kg/m.m.m") = 2500;
                                                //liquid density
  constant Real tanh_beta = 0.5;
                                // numerical "sharpness" for phase transition

  constant Real density_mult = 0.65 annotation(Dialog(tab = "General"));
  //0.55
...
protected
  ...
  algorithm
    solid := k_slope_mult*state.T + k_mult_solid; //20?
    liquid :=k_slope_mult*state.T + k_mult_liquid;
    lambda := tanh_beta*(Modelica.Math.tanh(2*Modelica.Constants.pi*(state.T-T_melt)/(T_meltplus-T_meltmin))+1)*liquid + tanh_beta*(Modelica.Math.tanh(-2*Modelica.Constants.pi*(state.T-T_melt)/(T_meltplus-T_meltmin))+1)*solid;
